...
namespace UltraVideoEditor
{
    public class RenderEngine
    {
            ...
            try
            {
                ...
                foreach (var item in sortedItems)
                {
                    ...
                    else if (isVideo)
                    {
                        ...
                        // PATCH 8 — Kids Edition Color Consistency
                        // Normalizujemo i brightness i saturation — sprečavamo "TV kanal" skok između klipova
                        ...
                        // PATCH 8 — Saturation clamp: sprečava prejakih/prebljedih klipova
                        // Ciljamo saturation band 1.05–1.20 (živo, ali ne neon)
                        // Dodajemo globalni clamp filtar UVEK za dečiji video
                        string satClampFilter = ",eq=saturation=1.10:contrast=1.02";
                        ...
                        videoVf = string.IsNullOrEmpty(moodFilter)
                            ? $"{scaleFilter},{baseNormalize}{colorMatchFilter}{wwbFilter}{fpsNormalize}"
                            : $"{scaleFilter},{baseNormalize},{moodFilter}{colorMatchFilter}{wwbFilter}{fpsNormalize}";
                        ...
                        if (!fastRender)
                        {
                            try
                            {
                                if (isStaticClip && item.Duration >= 2.0)
                                {
                                    ...
                                    videoVf = string.IsNullOrEmpty(moodFilter)
                                        ? $"{staticKB},{baseNormalize}{colorMatchFilter}{wwbFilter}"
                                        : $"{staticKB},{baseNormalize},{moodFilter}{colorMatchFilter}{wwbFilter}";
                                }
                                ...
                                if (item.Duration >= 3.0 && item.Duration <= 12.0 && !isStaticClip)
                                {
                                    ...
                                    _ = "center"; // zone uklonjena - nije korištena
                                    string xExpr = $"{midX}";
                                    string yExpr = $"{midY}";
                ...
                if (videoFiles.Count > 1)
                {
                    try
                    {
                        // Hybrid crossfade: 1.0s između foto↔video tipova, inače 0.3s
                        static bool isPhotoTag(string t) =>
                            t == "Emotional" || t == "Portrait";
                        ...
                        var fadeDurations = new List<double>();
                        for (int fi = 0; fi < orderedItems.Count - 1; fi++)
                        {
                            string tagA = orderedItems[fi].ContentTag ?? "";
                            string tagB = orderedItems[fi + 1].ContentTag ?? "";
                            bool aIsPhoto = isPhotoTag(tagA);
                            bool bIsPhoto = isPhotoTag(tagB);
                            // Prelaz foto↔video ili video↔foto = 1.0s, inače 0.2s (PATCH 11 — smooth 0.2s)
                            fadeDurations.Add((aIsPhoto != bIsPhoto) ? 1.0 : 0.2);
                        }

                        crossfadedVideo = Path.Combine(tempDir, "crossfaded.mp4");
                        crossfadedVideo = await ApplyCrossfade(videoFiles, crossfadedVideo, fadeDurations, cancellationToken);
                    }
                    catch (Exception ex)
                    {
                ...
                if (audio != null && File.Exists(audio.Path))
                {
                    ...
                    if (!string.IsNullOrEmpty(subtitleFile) && File.Exists(subtitleFile))
                    {
                        ...
                        argsFinal = $"-nostdin -f concat -safe 0 -i \"{concatFile}\" -i \"{tempAudioPath}\" " +
                                    $"-vf \"subtitles='{escapedSub}':force_style='FontSize=26,PrimaryColour=&H00FFFFFF,OutlineColour=&H00000000,Outline=5,Shadow=4,ShadowColour=&HAA000000,Bold=1,Alignment=2,MarginV=50,BorderStyle=3'\" " +
            ...
            CancellationToken ct)
        {
            ...
            {
                double fadeDuration = (fadeDurations != null && i - 1 < fadeDurations.Count)
                    ? fadeDurations[i - 1]
                    : 0.2; // PATCH 11: default crossfade 0.2s
