...
namespace UltraVideoEditor
{
    public partial class MainWindow : Window
    {
        ...
        private string _freesoundApiKey = "";
        private FreesoundClient _freesoundClient;
        ...
        public MainWindow()
        {
            ...
            {
                InitializeComponent();
                // Učitaj Freesound API ključ
                _freesoundApiKey = GetFreesoundApiKey();
                if (!string.IsNullOrEmpty(_freesoundApiKey))
                {
                    _freesoundClient = new FreesoundClient(_freesoundApiKey);
                    LogMessage("Freesound API inicijalizovan", false);
                }
                else
                {
        ...
        private void ApplyLanguage()
        {
            ...
            // ── TOOLBAR DUGMAD ───────────────────────────────────────
            if (btnRenderTool != null) btnRenderTool.Content = "🎬 " + L("render");
            if (btnNew != null) btnNew.Content = "📄 " + L("new_project");
            if (btnOpen != null) btnOpen.Content = "📂 " + L("open_project");
            if (btnSave != null) btnSave.Content = "💾 " + L("save_project");
            if (btnUndoTool != null) btnUndoTool.Content = "↩ " + L("undo");
            if (btnRedoTool != null) btnRedoTool.Content = "↪ " + L("redo");
            if (btnDeleteTool != null) btnDeleteTool.Content = "🗑 " + L("delete");
            if (btnCutTool != null) btnCutTool.Content = "✂ " + L("cut");
            if (btnCopyTool != null) btnCopyTool.Content = "📋 " + L("copy");
            if (btnPasteTool != null) btnPasteTool.Content = "📌 " + L("paste");

            // ── AI TAB - LABELE ──────────────────────────────────────
            if (txtAIVoice != null) txtAIVoice.Text = "🗣 " + L("ai_voice");
            if (txtVoiceTextLabel != null) txtVoiceTextLabel.Text = L("voice_text");
            if (txtAIFrames != null) txtAIFrames.Text = "🎨 " + L("ai_frames");
            if (txtAIPromptLabel != null) txtAIPromptLabel.Text = L("ai_prompt");
            if (txtAITranscribe != null) txtAITranscribe.Text = "🎤 " + L("ai_transcribe");
            if (txtTranscribeLabel != null) txtTranscribeLabel.Text = L("transcribe_label");
            if (txtAILyrics != null) txtAILyrics.Text = "🎤 " + L("ai_lyrics");
            if (txtLyricsLabel != null) txtLyricsLabel.Text = L("lyrics_text");
            if (txtSelectAudioLabel != null) txtSelectAudioLabel.Text = L("select_audio");

            // ── AI TAB - DUGMAD ──────────────────────────────────────
            if (btnGenerateVoice != null) btnGenerateVoice.Content = "🗣 " + L("generate_voice");
            if (btnGenerate != null) btnGenerate.Content = "🎨 " + L("generate_frames");
            if (btnTranscribe != null) btnTranscribe.Content = "🎤 " + L("transcribe_button");
            if (btnSyncLyrics != null) btnSyncLyrics.Content = "🎤 " + L("sync_lyrics");
            if (btnAddSyncedSubtitles != null) btnAddSyncedSubtitles.Content = "➕ " + L("add_subtitles");

            // ── EFEKTI TAB ───────────────────────────────────────────
            if (txtVideoEffects != null) txtVideoEffects.Text = "🎬 " + L("video_effects");
            if (txtBrightness != null) txtBrightness.Text = L("brightness");
            if (txtContrast != null) txtContrast.Text = L("contrast");
            if (txtBlur != null) txtBlur.Text = L("blur");
            if (txtAudioEffects != null) txtAudioEffects.Text = "🎵 " + L("audio_effects");
            if (txtBass != null) txtBass.Text = L("bass");
            if (txtTreble != null) txtTreble.Text = L("treble");
            if (txtReverb != null) txtReverb.Text = L("reverb");
            if (btnApplyEffects != null) btnApplyEffects.Content = "💾 " + L("apply_effects");

            // ── TITLOVI TAB ──────────────────────────────────────────
            if (txtSubtitleTextLabel != null) txtSubtitleTextLabel.Text = L("subtitle_text");
            if (txtSubStartLabel != null) txtSubStartLabel.Text = L("subtitle_start");
            if (txtSubEndLabel != null) txtSubEndLabel.Text = L("subtitle_end");
            if (btnAddSubtitle != null) btnAddSubtitle.Content = "➕ " + L("add_subtitle");
            if (btnClearSubtitles != null) btnClearSubtitles.Content = "🗑 " + L("clear_subtitles");

            // ── JAWS AutomationProperties ────────────────────────────
            // Ažuriraj accessibility opise ključnih elemenata
            if (txtAIPrompt != null)
                System.Windows.Automation.AutomationProperties.SetName(
                    txtAIPrompt, L("ai_prompt"));
            if (btnGenerate != null)
                System.Windows.Automation.AutomationProperties.SetName(
        ...
        private string GetFreesoundApiKey() => GetApiKey("freesound");
        private void SaveFreesoundApiKey(string key) => SaveApiKey("freesound", key);
        ...
        private async void TranscribeAudio_Click(object sender, RoutedEventArgs e)
        {
            ...
            string apiToken = GetHuggingFaceApiKey();
            if (string.IsNullOrEmpty(apiToken))
            {
                var dialog = new ApiKeyDialog("huggingface", "Hugging Face API token za transkripciju");
                if (dialog.ShowDialog() == true && !string.IsNullOrEmpty(dialog.ApiKey))
                {
                    apiToken = dialog.ApiKey;
            ...
            try
            {
                ...
                // Lokalni Whisper — ignorisemo apiToken (vise nije potreban)
...
JSON treba da sadrži listu 'raspored' gdje svaki element ima: 'animacija_index' (broj od 1 do {animacije.Count}), 'pocetak' (broj u sekundama), 'kraj' (broj u sekundama), 'razlog' (kratak opis na srpskom).";
        ...
        private void SetupTransitionContextMenus()
        {
            ...
            var miApply = new MenuItem { Header = "Primeni na selektovani klip" };
            miApply.Click += (s, ev) => ApplyTransitionToSelectedClip();
            var miApplyAll = new MenuItem { Header = "Primeni na sve prelaske" };
            miApplyAll.Click += (s, ev) => ApplyTransitionToAll();
            var miDesc = new MenuItem { Header = "Procitaj opis efekta (JAWS)" };
        ...
        private void InitNativeListView()
        {
            try
            {
                if (wfhTimeline?.Child is WinForms.ListView listView)
                {
                    ...
                    nativeListView.Columns.Add("Br.", 50, WinForms.HorizontalAlignment.Center);
                    ...
                    // Nakon "🔊 Pročitaj audio opis" dodajte:
                    contextMenu.Items.Add(new WinForms.ToolStripSeparator());
                    contextMenu.Items.Add("🎨 Dodaj tekst na sliku...", null, (s, e) => AddTextToImage_Click(null, null));
        ...
        private async void btnAutoArrange_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                // Inicijalizuj Freesound klijent ako su zvukovi uključeni
                // Inicijalizuj Freesound klijent ako su zvukovi uključeni
                FreesoundClient freesoundClient = null;
                if ((dialog.EnableTransitionSounds || dialog.EnableAmbientSounds) && !string.IsNullOrEmpty(dialog.FreesoundApiKey))
                {
                    freesoundClient = new FreesoundClient(dialog.FreesoundApiKey);
                    LogMessage("✅ Freesound klijent inicijalizovan za preuzimanje zvukova", true);

                    // Sačuvaj ključ za buduće korištenje
                    SaveFreesoundApiKey(dialog.FreesoundApiKey);
                }
                ...
                for (int i = 0; i < images.Count; i++)
                {
                    ...
                    if (dialog.EnableTransitionSounds && freesoundClient != null && i < images.Count - 1)
                    {
                        string transitionType = i % 2 == 0 ? "whoosh" : "pop";
                        string soundPath = await freesoundClient.GetTransitionSound(transitionType, _tempVideoFolder);
                ...
                // Ambijentalni zvukovi (ako su uključeni)
                // Ambijentalni zvukovi (ako su uključeni)
                if (dialog.EnableAmbientSounds && freesoundClient != null)
                {
                    LogMessage("Preuzimanje ambijentalnih zvukova...", true);
                    var ambientSounds = new List<TimelineItem>();

                    // Dodaj ambijentalne zvukove tokom cijelog videa (osim za intro/outro)
                    ...
                    if (ambientEnd > ambientStart)
                    {
                        ...
                        {
                            string soundPath = await freesoundClient.GetAmbientSound(ambientType, _tempVideoFolder);
                            if (!string.IsNullOrEmpty(soundPath))
                            {
                                var ambientItem = new TimelineItem
                                {
                                    ...
                                    Type = "Audio",
                                    Volume = 15,  // Niska jačina, ne smeta pjesmi
                                    TrackIndex = 2,
                                    VideoEffect = new VideoEffectData()
                                };
