    ...
    public partial class VideoEngineDialog : Window
    {
        ...
        private async void OnLoaded(object sender, RoutedEventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(_freesoundKey))
            {
                ...
                {
                    txtFreesoundInfo.Text       = "Freesound aktivan - ambijentalni zvukovi ce biti preuzeti";
            ...
            else
            {
                if (txtFreesoundInfo != null)
                {
                    txtFreesoundInfo.Text = "Freesound: unesi API key u Podesavanja -> API Kljucevi";
                    txtFreesoundInfo.Foreground = System.Windows.Media.Brushes.Orange;
                }
                if (chkAmbient != null) chkAmbient.IsEnabled = false;
            }

            // Pitaj za Freesound key ako nije podesen
            await PromptFreesoundKeyIfMissing();
        ...
        private async void btnGenerate_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                // ── Korak 2b: Preuzmi ambijentalni zvukovi ────────────
                var ambientFiles = new Dictionary<int, string>(); // shotIndex -> ambientPath
                if (_freesound != null && downloaded.Count > 0 && chkAmbient?.IsChecked == true)
                {
                    Announce("Preuzimam ambijentalne zvukove...", 93);
                    string tempDir = Path.GetDirectoryName(downloaded[0].path);

                    // Grupiši po sceni - jedan ambient po sceni (ne po kadru)
                    ...
                    for (int i = 0; i < downloaded.Count; i++)
                    {
                        ...
                        string ambPath = await _freesound.GetAmbientForScene(
                            context, shot.Duration * 3, tempDir, _cts.Token);
                        if (!string.IsNullOrEmpty(ambPath))
                            ambientFiles[i] = ambPath;
                    }
                    Announce($"Preuzeto {ambientFiles.Count} ambijentalnih zvukova.", 96);
                }
        ...
        // ── Helpers ──────────────────────────────────────────────────
        [System.Runtime.Versioning.SupportedOSPlatform("windows")]
        private async Task PromptFreesoundKeyIfMissing()
        {
            if (!string.IsNullOrEmpty(_freesoundKey)) return;

            string msg = "Freesound API kljuc nije podesen." + Environment.NewLine + Environment.NewLine +
                "Freesound.org je besplatan servis za ambijentalne zvukove (priroda, park, snijeg, cafe...)." + Environment.NewLine + Environment.NewLine +
                "Registruj se na freesound.org, idi na Account -> API Key." + Environment.NewLine + Environment.NewLine +
                "Da li zelis da unesesh API kljuc sada?";

            var result = WpfMessageBox.Show(msg, "Freesound API", MessageBoxButton.YesNo, MessageBoxImage.Question);

            if (result == MessageBoxResult.Yes)
            {
                string dlgMsg = "Freesound API kljuc" + Environment.NewLine + Environment.NewLine +
                    "1. Idi na freesound.org" + Environment.NewLine +
                    "2. Registruj se (besplatno)" + Environment.NewLine +
                    "3. Account -> Edit Profile -> API Applications" + Environment.NewLine +
                    "4. Kreiraj novu aplikaciju i kopiraj API key";
                var dlg = new ApiKeyDialog("freesound", dlgMsg);
                ...
                if (dlg.ShowDialog() == true && !string.IsNullOrEmpty(dlg.ApiKey))
                {
                    ...
                    if (chkAmbient != null) chkAmbient.IsEnabled = true;
                    Announce("Freesound API kljuc sacuvan. Ambijentalni zvukovi su aktivni.");
                }
                else
                {
                    Announce("Freesound nije podesen. Kreiranje ce raditi bez ambijentalnih zvukova.");
                    if (chkAmbient != null)
                    {
                        chkAmbient.IsChecked = false;
                        chkAmbient.IsEnabled = false;
                    }
                }
            }
            else
            {
                ...
                Announce("Freesound preskocen. Mozes ga podesiti kasnije u Podesavanja -> API kljucevi.");
            }
